...
class Player(Bot):
    ...
    def calcualte_strength(self, hole, board, iters):
        ...
        for _ in range(iters): #take 'iters' samples
            ...
            _COMM = 5 - len(old_community) #the number of cards we need to draw
            _OPP = 2

            draw = deck.peek(_COMM + _OPP)

            opp_hole = draw[: _OPP]
            community = old_community + draw[_OPP: ]
    ...
    def get_actions(self, game_state, round_state, active):
        ...
        for i in range(NUM_BOARDS):
            if AssignAction in legal_actions[i]:
                ...
                # Current boards on the table
                comm = [item for item in board_cards[i] if item]
